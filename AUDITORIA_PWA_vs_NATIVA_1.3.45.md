...
- **F1 · "6 activos pero límite 5" → NO es bug (verificado por Code).** `checkPlanLimit('add_portfolio')` (`index.html:4095-4098`) bloquea cuando `currentCount >= 5` y muestra "límite 5" — correcto. Tener 6 con FREE es un **artefacto de downgrade**: items agregados bajo un plan pago no se borran al bajar a FREE; el límite aplica a NUEVAS adiciones. Comportamiento esperado, no inconsistencia.
...
- **F5 · Modal de agregar (Watchlist) se corre/corta con el teclado — CONFIRMADO (mismo origen que M1).** `wlOpenAddModal` (`aurex-features.js:2028`) es un bottom-sheet `position:fixed;...;align-items:flex-end` con un input de búsqueda; al abrir el teclado, sin manejo de `safe-area`/viewport visible, se desplaza/corta. AJUSTE: igual que M1 (manejar teclado + `dvh`/safe-area).
